...
public class RecordVideoActivity extends AppCompatActivity {
    ...
    private static final int REQUEST_CAMERA_RECORD_AUDIO = 1001;
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        switch (requestCode) {
            case REQUEST_CAMERA_RECORD_AUDIO: {
                //todo 判断权限是否已经授予
                for (int i = 0; i < grantResults.length; i++) {
                    int state = grantResults[i];
                    if (state == PackageManager.PERMISSION_GRANTED) {
                        Toast.makeText(RecordVideoActivity.this, permissions[i] + " permission granted",
                                Toast.LENGTH_SHORT).show();
                    } else if (state == PackageManager.PERMISSION_DENIED) {
                        Toast.makeText(RecordVideoActivity.this, permissions[i] + " permission denied",
                                Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
